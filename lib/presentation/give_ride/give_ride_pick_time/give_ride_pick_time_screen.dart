...
import 'package:share_way_frontend/presentation/give_ride/give_ride_pick_time/bloc/give_ride_pick_time_bloc.dart';
import 'package:share_way_frontend/presentation/give_ride/give_ride_pick_time/bloc/give_ride_pick_time_state.dart';

class GiveRidePickTimeScreen extends StatefulWidget {
  final CreateGiveRideInput data;
  const GiveRidePickTimeScreen({super.key, required this.data});

  @override
  State<GiveRidePickTimeScreen> createState() => _GiveRidePickTimeScreenState();
}

class _GiveRidePickTimeScreenState extends State<GiveRidePickTimeScreen> {
  late GiveRidePickTimeBloc bloc;

  @override
  void initState() {
    bloc = GiveRidePickTimeBloc();
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      ...
      child: BlocListener<GiveRidePickTimeBloc, GiveRidePickTimeState>(
        ...
        child: BlocBuilder<GiveRidePickTimeBloc, GiveRidePickTimeState>(
  ...
  Widget _buildLocationInputField() {
    return BlocBuilder<GiveRidePickTimeBloc, GiveRidePickTimeState>(
      builder: (context, state) {
        return Container(
          ...
          child: ListView.separated(
            shrinkWrap: true,
            itemCount: state.createGiveRideInput?.locationList?.length ?? 0,
                ...
                children: [
                  Column(
                    ...
                    children: [
                      ...
                      if (index !=
                          (state.createGiveRideInput?.locationList?.length ??
                                  0) -
                              1)
                        _buildDashedLine()
                      else
                  ...
                  Expanded(
                    child: TextFormField(
                      initialValue:
                          state.createGiveRideInput?.locationList?[index].name,
                      enabled: false,
